...
export interface AIService {
  generateChatReply(message: string): Promise<ChatResponse>;
  generateCrowdPrediction(zones: { name: string; density: number; current: number; capacity: number }[]): Promise<PredictResponse>;
}
...
export class GeminiAIService implements AIService {
  ...
  /**
   * Generates a conversational reply using Gemini or local fallback.
   */
  async generateChatReply(message: string): Promise<ChatResponse> {
    try {
      const sanitizedMessage = sanitizeInput(message);
      if (!sanitizedMessage) {
        return { success: false, reply: 'Message cannot be empty.' };
      }
  ...
  /**
   * Predicts stand density and provides operational advice using Gemini or Heuristics.
   */
  async generateCrowdPrediction(zones: { name: string; density: number; current: number; capacity: number }[]): Promise<PredictResponse> {
    if (!Array.isArray(zones) || zones.length === 0) {
      return {
        success: false,
        predictions: [],
        generalAdvice: 'Invalid zone telemetry payload.',
      };
    }
    try {
      if (this.apiKey) {
      ...
      // Local heuristic fallback prediction engine using stable formula (determinism)
      const predictions: PredictionItem[] = zones.map((zone, idx) => {
        // Deterministic offset based on zone parameters (avoid pure Math.random inside service logic if possible)
        const seedVal = zone.name.length + zone.density + idx;
        const deterministicFluctuation = Math.floor(Math.sin(seedVal) * 6);
        const predictedDensity = Math.min(100, Math.max(10, zone.density + deterministicFluctuation));
